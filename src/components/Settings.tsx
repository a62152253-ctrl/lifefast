...
import {
  LogOut, Bell, Shield, Paintbrush, Globe, Info, ChevronRight, CreditCard,
  Sparkles, Share2, Mail, Check, X, Users, RefreshCw, Sun, Moon, Download,
...
const AVATAR_EMOJIS = ['😎','🦊','🐺','🦋','🌟','⚡','🔥','🌊','🍀','🦅','🌙','🎯','💎','🚀','🎨','👑','🐉','🦁','🦸','🦹','🧙','🧚','🧛','🧟','🧞','🧜','🧝','👁️','🧠','🫀','🫁','🦸‍♀️','🦸‍♂️','🦹‍♀️','🦹‍♂️','🧙‍♀️','🧙‍♂️','🧚‍♀️','🧚‍♂️','🧛‍♀️','🧛‍♂️','🧟‍♀️','🧟‍♂️','🧞‍♀️','🧞‍♂️','🧜‍♀️','🧜‍♂️','🧝‍♀️','🧝‍♂️'];
const AVATAR_COLORS = [
  { id: 'indigo',  cls: 'bg-indigo-500',  label: 'Indygo',     hex: '#6366f1' },
  { id: 'violet',  cls: 'bg-violet-500',  label: 'Fiolet',     hex: '#8b5cf6' },
  { id: 'rose',    cls: 'bg-rose-500',    label: 'Róż',        hex: '#f43f5e' },
  { id: 'amber',   cls: 'bg-amber-500',   label: 'Bursztyn',   hex: '#f59e0b' },
  { id: 'emerald', cls: 'bg-emerald-500', label: 'Szmaragd',   hex: '#10b981' },
  { id: 'sky',     cls: 'bg-sky-500',     label: 'Błękit',     hex: '#0ea5e9' },
  { id: 'orange',  cls: 'bg-orange-500',  label: 'Pomarańcz',  hex: '#f97316' },
  { id: 'slate',   cls: 'bg-slate-700',   label: 'Grafit',     hex: '#334155' },
  { id: 'red',     cls: 'bg-red-500',     label: 'Czerwony',   hex: '#ef4444' },
  { id: 'green',   cls: 'bg-green-500',   label: 'Zielony',    hex: '#22c55e' },
  { id: 'blue',    cls: 'bg-blue-500',    label: 'Niebieski',  hex: '#3b82f6' },
  { id: 'purple',  cls: 'bg-purple-500',  label: 'Purpurowy',  hex: '#a855f7' },
  { id: 'pink',    cls: 'bg-pink-500',    label: 'Różowy',     hex: '#ec4899' },
  { id: 'yellow',  cls: 'bg-yellow-500',  label: 'Żółty',      hex: '#eab308' },
  { id: 'teal',    cls: 'bg-teal-500',    label: 'Morski',     hex: '#14b8a6' },
  { id: 'cyan',    cls: 'bg-cyan-500',    label: 'Cyjan',      hex: '#06b6d4' },
...
export default function Settings() {
  ...
  useEffect(() => {
    ...
    return () => { unsubProfile(); unsubReceived(); unsubSent(); };
  }, [user]);
  ...
  const handleExportData = async (format: 'json' | 'csv') => {
    ...
    if (format === 'json') exportToJSON(allData, `lifeflow_export_${user.uid}`);
    else exportToCSV(flatData, `lifeflow_export_${user.uid}`);
    setIsLoading(false);
  };
    ...
    <div className="space-y-10 pb-32">
      {error && (
        <div className="mb-6 p-4 bg-rose-50 border border-rose-200 rounded-xl">
          <div className="flex items-center gap-3">
            ...
            <button type="button" onClick={() => setError(null)} className="ml-auto text-rose-600 hover:text-rose-800 text-sm font-medium">
              X
            </button>
          </div>
        </div>
        ...
        <section className="space-y-4">
          ...
          <Card className="flex flex-col sm:flex-row items-center gap-6 p-8 bg-gradient-to-br from-white to-gray-50 border-gray-100 shadow-xl shadow-indigo-500/5">
            <div className="relative shrink-0">
              <div className={cn('w-24 h-24 rounded-[2rem] flex items-center justify-center text-white shadow-2xl transition-transform duration-500', getAvatarColor(profile?.avatarColor || 'indigo').cls)}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="space-y-4">
                  <div className="flex items-center gap-3 mb-2">
                    <div className="p-3 bg-amber-50 text-amber-600 rounded-xl"><Share2 size={24} /></div>
          ...
          <section className="space-y-4">
            ...
            <Card className="divide-y divide-gray-50 p-0 overflow-hidden shadow-xl shadow-gray-200/50">
              ...
              <div onClick={() => setShowLanguage(true)} className="cursor-pointer">
                <SettingItem icon={Globe} title={t('settings.language')} subtitle={language === 'pl' ? 'Polski (Polska)' : language === 'en' ? 'English (UK)' : 'Deutsch (DE)'} />
              </div>
      ...
      <Modal isOpen={showProfile} onClose={() => setShowProfile(false)} title="Edytuj profil">
        <form onSubmit={saveProfile} className="space-y-5">
          <div className="flex items-start gap-5 p-4 bg-gray-50 rounded-2xl border border-gray-100">
            <div className={cn('w-20 h-20 shrink-0 rounded-2xl flex items-center justify-center text-4xl shadow-lg transition-all duration-200', getAvatarColor(editColor).cls)}>
              {editEmoji}
            </div>
            ...
            <div className="grid grid-cols-9 gap-1.5">
              {AVATAR_EMOJIS.map(em => (
                <button
                  ...
                  className={cn('w-9 h-9 rounded-xl text-xl flex items-center justify-center transition-all border-2', editEmoji === em ? 'border-indigo-500 bg-indigo-50 scale-110 shadow-md' : 'border-gray-100 bg-gray-50 hover:border-gray-200')}
                >
                  {em}
                </button>
              ))}
            ...
            <div className="flex gap-2 flex-wrap">
              {AVATAR_COLORS.map(c => (
                <button
                  ...
                  className={cn('w-8 h-8 rounded-xl transition-all border-2', c.cls, editColor === c.id ? 'scale-110 border-white shadow-lg ring-2 ring-offset-1 ring-gray-400' : 'border-transparent opacity-60 hover:opacity-100 hover:scale-105')}
                />
              ))}
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3 pt-1">
            <Button type="button" variant="ghost" onClick={() => setShowProfile(false)}>Anuluj</Button>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <ul className="space-y-2">
              {['Listy zadań i zakupów', 'Nawyki i statystyki realizacji', 'Historię Twojego budżetu'].map(item => (
      ...
      <Modal isOpen={showHelp} onClose={() => setShowHelp(false)} title="Centrum Pomocy">
        <div className="space-y-6">
          <div className="space-y-4">
            {[
              { q: 'Jak działa synchronizacja?', a: 'Wpisz e-mail partnera w sekcji "Synchronizacja Kont". Po zaakceptowaniu zaproszenia, Wasze zakupy i zadania będą wspólne w czasie rzeczywistym.' },
              { q: 'Czym jest LifeFlow AI?', a: 'To Twój osobisty asystent na głównym panelu, który analizuje Twoje zadania i nawyki, by dawać Ci spersonalizowane rady każdego dnia.' },
              { q: 'Gdzie jest eksport do ZIP?', a: 'Ta funkcja jest dostępna w menu ustawień platformy Google AI Studio (ikona zębatki na górnym pasku).' },
            ].map(({ q, a }) => (
              <div key={q} className="p-4 bg-gray-50 rounded-2xl border border-gray-100">
                <h4 className="font-bold text-gray-900 mb-1">{q}</h4>
      ...
      <Modal isOpen={showNotifications} onClose={() => setShowNotifications(false)} title="Powiadomienia">
        <div className="space-y-8">
          <div className="space-y-6">
            <NotificationToggle title="Powiadomienia Push" subtitle="Otrzymuj alerty o zadaniach i planach na żywo"
            ...
            <NotificationToggle title="Inteligentne Przypomnienia" subtitle="Alerty o nadchodzących spotkaniach z planu dnia"
            ...
            <NotificationToggle title="Nawyki i Rutyna" subtitle="Motywujące przypomnienia o Twoich nawykach"
            ...
            <NotificationToggle title="Synchronizacja Zakupów" subtitle="Powiadomienia o zmianach we wspólnej liście"
      ...
      <Modal isOpen={showAppearance} onClose={() => setShowAppearance(false)} title="Wygląd aplikacji">
        <div className="space-y-8">
          <div className="grid grid-cols-2 gap-4">
            <div onClick={() => setTheme('default')} className={cn("p-4 rounded-2xl border-2 text-center cursor-pointer transition-all", theme === 'default' ? "border-indigo-600 bg-white" : "border-gray-100 bg-gray-50 opacity-60")}>
              <div className="w-12 h-12 bg-indigo-600/10 rounded-xl mx-auto mb-3 flex items-center justify-center text-indigo-600"><Sun size={24} /></div>
              <p className="font-bold text-gray-900 text-sm">Jasny</p>
            </div>
            <div onClick={() => setTheme('dark')} className={cn("p-4 rounded-2xl border-2 text-center cursor-pointer transition-all", theme === 'dark' ? "border-indigo-600 bg-gray-900" : "border-gray-900/10 bg-gray-900 opacity-60")}>
              <div className="w-12 h-12 bg-white/10 rounded-xl mx-auto mb-3 flex items-center justify-center text-white"><Moon size={24} /></div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[{ id: 'ocean', label: 'Ocean', color: 'bg-blue-500' }, { id: 'forest', label: 'Las', color: 'bg-emerald-500' }, { id: 'sunset', label: 'Zachód', color: 'bg-orange-500' }].map(m => (
                <button key={m.id} type="button" onClick={() => setTheme(m.id as any)} className={cn("flex flex-col items-center gap-2 p-3 rounded-2xl border-2 transition-all", theme === m.id ? "border-indigo-600 bg-white shadow-lg" : "border-gray-50 bg-gray-50")}>
          ...
          <div className="space-y-4 pt-4 border-t border-gray-50">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button type="button" onClick={() => setDeviceType('desktop')} className={`py-3 rounded-xl border-2 transition-all font-bold text-xs ${deviceType === 'desktop' ? 'border-indigo-600 bg-indigo-50 text-indigo-600' : 'border-gray-100 text-gray-400 hover:border-gray-200'}`}>Komputer</button>
              <button type="button" onClick={() => setDeviceType('mobile')} className={`py-3 rounded-xl border-2 transition-all font-bold text-xs ${deviceType === 'mobile' ? 'border-indigo-600 bg-indigo-50 text-indigo-600' : 'border-gray-100 text-gray-400 hover:border-gray-200'}`}>Telefon (1:1)</button>
            </div>
          </div>
      ...
      <Modal isOpen={showLanguage} onClose={() => setShowLanguage(false)} title={t('settings.language')}>
        <div className="space-y-4">
          <div className="grid gap-3">
            {[{ code: 'pl', name: 'Polski', flag: '🇵🇱' }, { code: 'en', name: 'English', flag: '🇬🇧' }, { code: 'de', name: 'Deutsch', flag: '🇩🇪' }].map(lang => (
              <button
                key={lang.code}
                type="button"
                onClick={() => setLanguage(lang.code as any)}
                className={cn("p-4 rounded-2xl border-2 flex items-center justify-between transition-all", language === lang.code ? "border-indigo-600 bg-indigo-50" : "border-gray-100 bg-white hover:border-gray-200")}
              >
                <div className="flex items-center gap-4">
      ...
      <Modal isOpen={showSubscription} onClose={() => setShowSubscription(false)} title="Twój Plan">
        <div className="space-y-8">
          ...
          <div className="space-y-4">
            ...
            {['Nielimitowana liczba synchronizowanych kont', 'LifeFlow AI Personal Insight (Gemini Powered)', 'Nielimitowane nawyki i grupy zadań', 'Wsparcie priorytetowe 24/7'].map(perk => (
              <div key={perk} className="flex items-center gap-3 text-sm text-gray-600">
                <div className="w-5 h-5 bg-emerald-50 text-emerald-600 rounded-full flex items-center justify-center shrink-0"><Check size={12} /></div>
                {perk}
              </div>
            ))}
          </div>
      ...
      <Modal isOpen={showAiControl} onClose={() => setShowAiControl(false)} title="Zarządzanie AI">
        <div className="space-y-8">
          ...
          <div className="space-y-6">
            <NotificationToggle title="AI Task Specialist" subtitle="Rozbijanie zadań na podzadania i kroki" checked={true} onChange={() => {}} />
            <NotificationToggle title="AI Habit Strategist" subtitle="Spersonalizowany coaching i porady psychologiczne" checked={true} onChange={() => {}} />
            <NotificationToggle title="AI Food Scientist" subtitle="Szacowanie kalorii i wartości odżywczych" checked={true} onChange={() => {}} />
            <NotificationToggle title="AI Daily Executive" subtitle="Analiza produktywności na Dashboardzie" checked={true} onChange={() => {}} />
          </div>
...
function NotificationToggle({ title, subtitle, checked, onChange }: { title: string; subtitle: string; checked: boolean; onChange: (val: boolean) => void }) {
  return (
